...
public class TypeChecking extends DefaultVisitor {
	...
	public Object visit(Return node, Object param) {
		...
		if (node.getExpresion() == null) {
			...
			predicado(node.getFuncionEnLaQueEstoy().getTipo().getClass() == node.getExpresion().getTipo().getClass(),
	...
	public Object visit(Asignacion node, Object param) {
		...
		if (node.getIzq().getTipo() != null && node.getDer().getTipo() != null) {
			...
			predicado(node.getIzq().getTipo().getClass() == node.getDer().getTipo().getClass(), "Asignaci髇: valores de distinto tipo", node);
			predicado(
					node.getIzq().getTipo().getClass() != TipoStruct.class,
					"Asignaci髇: El valor de la izquierda no puede ser un struct", node.getIzq());
			
			if(node.getIzq().getTipo().getClass() == TipoArray.class 
					&& node.getDer().getTipo().getClass() == TipoArray.class) {	
				TipoArray izq = (TipoArray) node.getIzq().getTipo();
				TipoArray der = (TipoArray) node.getDer().getTipo();
				
				predicado(izq.getTipo().getClass() == TipoChar.class && der.getTipo().getClass() == TipoChar.class,
						"Asignacion: El tipo de los arrays debe ser tipo car醕ter.", node.getIzq());
				
				predicado( Integer.parseInt(der.getDimension()) >= Integer.parseInt(izq.getDimension()) ,
						"Asignaci髇: El array origen debe tener un numero de caracterers mayor o igual que el destino.", node.getIzq());
			}
		}
		return null;
	}
	...
	public Object visit(Print node, Object param) {
		...
		if(node.getExpresion().getTipo() != null) {
			...
			predicado(
			node.getExpresion().getTipo().getClass() != TipoStruct.class,
				"Print: el tipo no puede ser un struct", node);
			
			if(node.getExpresion().getTipo().getClass() == TipoArray.class) {
				
				TipoArray tipo = (TipoArray)node.getExpresion().getTipo();
				predicado(
						tipo.getTipo().getClass() == TipoChar.class,
							"Print: el tipo del array a devolver debe ser char", node);
	...
	public Object visit(InvocacionFuncionExpresion node, Object param) {
		...
		if (parametros.size() != node.getParametros().size()) {
			...
			boolean tipoParametro = true;
			for (int i = 0; i < parametros.size(); i++) {
				if (parametros.get(i).getTipo().getClass() != node.getParametros().get(i).getTipo().getClass())
					tipoParametro = false;
			}
			predicado(tipoParametro==true, "InvocacionFuncionExpresion: el tipo de los par醡etros no coincide",
					node.getStart());
		} 
