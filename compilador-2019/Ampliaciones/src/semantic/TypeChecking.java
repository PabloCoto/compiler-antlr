...
//	class OperadorTernario { Expresion ex1;  Expresion ex2;  Expresion ex3; }
	public Object visit(OperadorTernario node, Object param) {
		super.visit(node, param); //para que se evaluen las expresiones del operador 
		predicado(node.getEx1().getTipo().getClass() == TipoEntero.class, "OperadorTernario: La condici髇 debe ser booleana, en nuestro caso tipo entero");
		predicado(node.getEx2().getTipo().getClass() == node.getEx3().getTipo().getClass(), "OperadorTernario: Ambas alternativas deben ser del mismo tipo");
		node.setTipo(node.getEx2().getTipo());
		node.setModificable(false);
		return null;
	}
