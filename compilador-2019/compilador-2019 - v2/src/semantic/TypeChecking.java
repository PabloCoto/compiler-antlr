...
public class TypeChecking extends DefaultVisitor {
	...
	public Object visit(Asignacion node, Object param) {
		...
		predicado(node.getIzq().isModificable(), "Asignaci髇: valor de la izquierda no modificable", node.getIzq());
		predicado(node.getIzq().getTipo().getClass() != TipoStruct.class && node.getIzq().getTipo().getClass() != TipoArray.class, "Asignaci髇: El valor de la izquierda debe ser simple", node.getIzq());
		return null;
	}
	...
	public Object visit(AccesoArray node, Object param) {
		...
		predicado(node.getPosicion().getTipo().getClass()==TipoEntero.class, "AccesoArray: debe ser 韓dice entero", node);
		node.setModificable(true);
		return null;
	}
...
//		super.visit(node, param);
		...
		if(parametros.size() != node.getParametros().size()) {
			predicado(parametros.size() == node.getParametros().size(), "InvocacionFuncionExpresion: n鷐ero de argumentos no coincide", node);
		} else {
			...
			predicado(tipoParametro, "InvocacionFuncionExpresion: el tipo de los par醡etros no coincide",node.getStart());
		}    //FALTA LO DEL VOID
		return null;
	}
