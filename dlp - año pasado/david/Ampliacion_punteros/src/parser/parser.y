...
var_def:	p_type list_ident ';'		{ $$ = new ArrayList<Definition>(); addVarDefs((List<Definition>)$$, (Type)$1, (List<String>)$2, scanner.getLine()); }
			...
			| puntero list_ident';'		{ $$ = new ArrayList<Definition>(); addVarDefs((List<Definition>)$$, (PunteroType)$1, (List<String>)$2, scanner.getLine());}
			;
...
puntero:	type '*'									{ $$ = getPunteroDef((Type)$1, scanner.getLine());} 
			;
...
exp:	exp '+' exp                               		{ $$ = new ArithmeticOperation(scanner.getLine(), scanner.getColumn(), (Expression)$1, (Expression)$3, "+"); }
		...
		//AMPLIACION
		| '&' exp                  %prec ACCESO         { $$ = new PunteroAddressAccess(scanner.getLine(), scanner.getColumn(), (Expression)$2); }
		| '*' exp                  %prec ACCESO2        { $$ = new PunteroValueAccess(scanner.getLine(), scanner.getColumn(), (Expression)$2); }
		;
